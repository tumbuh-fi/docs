...
|---|---|---|
| **TumbuhRouter** | The user-facing entry layer; per-user position accounting; receipt-token mint and burn; stablecoin↔vault-asset swaps for deposits and withdrawals | Vault-asset accounting; strategy allocation; fee collection |
| **TumbuhVault** | The vault's value accounting; idle vault asset; performance-fee mints (when the new model is live); the all-time-high watermark | Stablecoins other than the vault asset; swap execution; per-user position tracking; strategy logic |
| **StrategyManager** | The strategy registry; allocation across active strategies; rebalancing; reward compounding; emergency exits | Vault-asset custody beyond transient pulls; performance fee; per-user positions |
...
| **TIDRX** | A soulbound 1:1 receipt for stablecoin deposits | Yield accrual; vault-position tracking |

## How the contracts connect

- The router is the only contract a regular user interacts with. It pulls stablecoin from the user, drives the swap, places the result into the vault, and issues the receipt token.
