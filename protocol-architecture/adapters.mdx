...
Adding a new yield protocol to Tumbuh is coordinated with the Tumbuh team. Most protocols only require pointing the existing `GenericERC4626Strategy` adapter at the protocol's ERC-4626 vault — no new contract is needed. Protocols that pay rewards through a non-standard distributor require a new adapter contract; that work is sequenced with the team rather than authored independently.
